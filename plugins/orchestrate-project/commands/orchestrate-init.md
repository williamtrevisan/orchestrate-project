...
Read the report. It carries the shipped tracker set, each tracker's probe verdict with the exact command and stderr, Compozy's presence and version against the pin, candidate gate commands with the manifest each came from, candidate conventions documents, and whether a configuration already exists.
...
| Compozy is not installed — install it? | The install command from the report, and "not now" | `compozy` resolves |
